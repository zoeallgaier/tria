...
calls per hold, and the fan only ever fades (no blur, no transform), so the
frames those calls cross are the compositor's rather than the main thread's. On the web the bookends do nothing and
